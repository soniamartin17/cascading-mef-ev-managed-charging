            ...
            for access_series_name in ['access_series', 'plugged_series']:
                ...
                for week in range(data.num_weeks):
                    ...
                    model.run_control_oneweek(week, name=control_folder_name, access_series_name=access_series_name, 
                                              objective_type='daytime', reg=1, dpdf=dpdf_total, mef_type=mef_type,
                ...
                for week in range(data.num_weeks):
                    ...
                    all_controlled_demand_l1.iloc[:,np.arange(1, num_vehicles+1)] = all_controlled_demand_l1.iloc[:,np.arange(1, num_vehicles+1)].where(all_controlled_demand_l1.iloc[:,np.arange(1, num_vehicles+1)]<l1_rate/(1+eta), 0)

                    all_controlled_demand_l2 = model.controlled_charging_demand[control_folder_name].copy(deep=True)
                    all_controlled_demand_l2.iloc[:,np.arange(1, num_vehicles+1)] = all_controlled_demand_l2.iloc[:,np.arange(1, num_vehicles+1)].where((all_controlled_demand_l2.iloc[:,np.arange(1, num_vehicles+1)]<=(l2_rate/(1+eta)+.01)), 0)

                    all_controlled_demand_l3 = model.controlled_charging_demand[control_folder_name].copy(deep=True)
                    all_controlled_demand_l3.iloc[:,np.arange(1, num_vehicles+1)]=all_controlled_demand_l3.iloc[:,np.arange(1, num_vehicles+1)].where(all_controlled_demand_l3.iloc[:,np.arange(1, num_vehicles+1)]>(l2_rate/(1+eta)+.01), 0)
                    ...
                    model.controlled_charging_demand[control_folder_name].loc[inds, 'l1_demand'] = all_controlled_demand_l1.loc[inds, np.arange(1, num_vehicles+1).astype(str)].multiply(dot_values.values).sum(axis=1)
                    model.controlled_charging_demand[control_folder_name].loc[inds, 'l2_demand'] = all_controlled_demand_l2.loc[inds, np.arange(1, num_vehicles+1).astype(str)].multiply(dot_values.values).sum(axis=1)
                    model.controlled_charging_demand[control_folder_name].loc[inds, 'l3_demand'] = all_controlled_demand_l3.loc[inds, np.arange(1, num_vehicles+1).astype(str)].multiply(dot_values.values).sum(axis=1)
