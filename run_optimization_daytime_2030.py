...
gds = {}
df = pd.read_csv('/scratch/users/sonia17/Data/audi_etron_ca_PROCESSED_withAccess_withSpeeds.csv', index_col=0)
folder_root = '/scratch/users/sonia17'
save_str = folder_root+'/Data/all_uncontrolled_demand'
gds[2019] = pickle.load(open('/scratch/users/sonia17/Data/generator_data_short_WECC_2019.obj', 'rb'))
gds[2020] = pickle.load(open('/scratch/users/sonia17/Data/generator_data_short_WECC_2020.obj', 'rb'))
...
results_date = '20231207' #set simulation date for saving files
folder = folder_root + '/Daytime_2030'
future_year = 2030
            ...
            for access_series_name in ['access_series', 'plugged_series']:
                ...
                for week in range(data.num_weeks):
                    ...
                    model.run_control_oneweek(week, name=control_folder_name, access_series_name=access_series_name, 
                                                objective_type='daytime', dpdf=dpdf_total, mef_type=mef_type,
                ...
                for week in range(data.num_weeks):
                    ...
                    total_controlled_demand_l1.iloc[:,np.arange(1, 749)] = total_controlled_demand_l1.iloc[:,np.arange(1, 749)].where(total_controlled_demand_l1.iloc[:,np.arange(1, 749)]<l1_rate/(1+eta), 0)
                    total_controlled_demand_l2 = model.controlled_charging_demand[control_folder_name].copy(deep=True)
                    total_controlled_demand_l2.iloc[:,np.arange(1, 749)] = total_controlled_demand_l2.iloc[:,np.arange(1, 749)].where(total_controlled_demand_l2.iloc[:,np.arange(1, 749)]<=(l2_rate/(1+eta)+.01), 0)
                    total_controlled_demand_l3 = model.controlled_charging_demand[control_folder_name].copy(deep=True)
                    total_controlled_demand_l3.iloc[:,np.arange(1, 749)] = total_controlled_demand_l3.iloc[:,np.arange(1, 749)].where(total_controlled_demand_l3.iloc[:,np.arange(1, 749)]>(l2_rate/(1+eta)+.01), 0)
                    
                    #multiply drivers by frequency
                    model.controlled_charging_demand[control_folder_name].loc[inds, 'l1_demand'] = total_controlled_demand_l1.loc[inds, np.arange(1, 749).astype(str)].multiply(dot_values.values).sum(axis=1)
                    model.controlled_charging_demand[control_folder_name].loc[inds, 'l2_demand'] = total_controlled_demand_l2.loc[inds, np.arange(1, 749).astype(str)].multiply(dot_values.values).sum(axis=1)
                    model.controlled_charging_demand[control_folder_name].loc[inds, 'l3_demand'] = total_controlled_demand_l3.loc[inds, np.arange(1, 749).astype(str)].multiply(dot_values.values).sum(axis=1)
