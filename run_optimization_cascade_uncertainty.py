...
df = pd.read_csv('Data/audi_etron_ca_PROCESSED_withAccess_withSpeeds.csv', index_col=0) #insert CSV of processed data
folder_root = 'Data' #None #insert file location here
save_str = folder_root+'/all_uncontrolled_demand'
#gds[2019] = pickle.load(open('Data/generator_data_short_WECC_2019.obj', 'rb'))
gds[2020] = pickle.load(open(folder_root + '/generator_data_short_WECC_2020.obj', 'rb'))
...
results_date = '20250115'
folder = folder_root + '/MEF_Uncertainty/'
for run_number in [np.mod(int(sys.argv[1])-1, 15)]:#range(15): #iterate to make multiple runs
#for run_number in range(15): #iterate to make multiple runs
    ...
    n_evs_added = nev_set[np.floor_divide(int(sys.argv[1])-1, 15)]
    
    print('-----'*5)
    #n_evs_added = nev_set[np.floor_divide(int(sys.argv[1])-1, 15)]
    #n_evs_added = nev_set[0]
    ...
    grid_u = GridModel(year=str(year), gd=gds[year])
    ...
    #grid dispatch for subsets of uncontrolled charging
    ...
    for signal_num, fraction in enumerate(uncontrolled_fraction):
        ...
        added_demand = added_demand.resample('H', on='datetime').sum().reset_index()
        added_demand.demand = (1/60)*added_demand.demand # as part of the hourly conversion
        tmp_added_demand = added_demand.copy(deep=True)
        ...
        if len(total_controlled_demand_running.total_demand) != 0:
            ...
            #initialize displacement and normal noise vectors
            displacement = np.random.uniform(0, 1, len(temp_ctrl_run.total_demand))
            normal_noise = np.random.normal(0, 0.25 * np.max(uncontrolled_demand_prior_group.demand), len(temp_ctrl_run.total_demand))

            #scale displacement so kWh are equal in 100% controlled and 75% controlled scenarios
            displacement = displacement  * (np.sum(temp_ctrl_run.total_demand) - (np.sum(0.75*temp_ctrl_run.total_demand) +np.sum(0.25 * uncontrolled_demand_prior_group.demand) + np.sum(normal_noise))) / np.sum(displacement)
            #sum these up for altered added demand
            added_demand.demand += 0.75*temp_ctrl_run.total_demand + 0.25 * uncontrolled_demand_prior_group.demand + normal_noise + displacement
        ...
        #run grid dispatch
        if len(added_demand_mw) > 0:
            grid1.run_dispatch(save_str=folder+'/'+folder_numevs+'/Controlled_'+control_folder_name+'/'+'results_'+str(year)+'_run'+str(run_number)+'_'+period_string+ '_'+str(signal_num), result_date = results_date, time_array=np.arange(minweek, maxweek+1)+1)
        else:
        ...
        for week in range(data.num_weeks):
            ...
            #set driver set for each week as the next group of EVs
        ...
        #update running total of demand
        if signal_num==0:
